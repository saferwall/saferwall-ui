<script lang="ts">
	...
	const moreProcessesToShow = 100;
	let treesToShow = moreProcessesToShow;
	$: debouncedTrees = trees.slice(0, treesToShow);
...
{:else if trees.length}
	<ul class="flex flex-col divide-y divide-line-surface {isChild && trees.length ? "border-t border-line-surface " : "pl-12"}">
		...
		{/each}
		{#if trees.length > debouncedTrees.length}
			<li class="flex justify-center">
				<button class="py-3 px-4 text-brand-text flex gap-2 items-center font-semibold" on:click={() => treesToShow += moreProcessesToShow}>
					Show more processes ({trees.length - debouncedTrees.length})
					<Icon name="arrow-down" size="size-3.5" class="text-inherit" />
				</button>
			</li>
		{/if}
	</ul>
