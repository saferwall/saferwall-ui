...
export const actions = {
    information: async ({ locals, request }) => {
        ...
        await new APIClient(locals.session).updateProfile(data);
        return true;
    },
    email: async ({ locals, request }) => {
        ...
        try {
            await new APIClient(locals.session).updateEmail({
                email,
                password,
                username
            });
    ...
    delete: async ({ locals }) => {
        ...
        try {
            await new APIClient(locals.session).deleteAccount(username);
            return { status: true };
        } catch (response: any) {
            return fail(400, await response.json());
        }
